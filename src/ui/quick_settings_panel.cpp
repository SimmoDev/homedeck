...
// track's nominal bounding box by roughly its own radius. Plain padding
// equal on every side (as this was originally) left the knob visually
// crowding the panel edge at 0%/100%.
...
// either side, no caption) were tried for volume alone - LVGL's bundled
// symbol font has a matching pair for volume
// (LV_SYMBOL_MUTE/LV_SYMBOL_VOLUME_MAX) but no "brightness" glyph at all
// (LV_SYMBOL_TINT/LV_SYMBOL_IMAGE are a paint drop and a picture frame,
// not brightness), so only one row could get icons - visually
// inconsistent, reverted. A unified icon pass for both is tracked as an
// M7 polish item instead of solving it once for volume alone (see
// docs/roadmap.md).
...
// to contain the knob instead, the same as before any per-control
// grouping existed.
