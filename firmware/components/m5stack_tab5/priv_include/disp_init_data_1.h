...
static const st7123_lcd_init_cmd_t disp_init_data_st7123[] = {
    ...
    // Upstream shipped delay_ms=0 on every command, including Sleep Out
    // (0x11) and Display On (0x29) - MIPI DCS requires >=120ms after
    // Sleep Out before further commands, and bsp_display_backlight_on()
    // (called by app code right after this table finishes) had no
    // settle time after Display On at all, letting the backlight turn
    // on before the panel had optically settled to its commanded colors
    // - see docs/decisions/ADR-0022-panel-init-settle-delay.md.
