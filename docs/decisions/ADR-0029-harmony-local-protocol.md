...
1. **Handshake** (plain HTTP, not yet the WebSocket): `POST
   ...
   **Requires an `Origin: http://sl.dhg.myharmony.com` header** - confirmed
   ...
   including it succeeds. Not a real browser origin; just what the hub's
   own validation checks for. The response carries `activeRemoteId`
...
2. **Control**: WebSocket to
   ...
   confirmed live, returning the reference hub's real device and activity
...
- The account email/username in the handshake response is real personal
  data returned over an unauthenticated local endpoint - a fact about the
...
- The WebSocket message shapes for activities/devices beyond the
  ...
  community-documented but not independently field-verified - real
