...
| `nvs_keys` | data/nvs_keys    | 4KB       | Reserved for NVS encryption keys under ESP-IDF's flash-encryption-based scheme — not the scheme [ADR-0010](ADR-0010-secret-storage.md) actually chose. The HMAC-peripheral scheme ADR-0010 decided on derives keys from eFuse at runtime and never touches this partition; it stays reserved (not reclaimed) in case a future [ADR-0018](ADR-0018-staged-security-hardening.md) Hardened tier ever combines NVS encryption with flash encryption |
